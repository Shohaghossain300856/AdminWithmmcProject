...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .header-left img { width: 65px; height: 65px; object-fit: cover; border-radius: 8px; background: white; padding: 4px; }
    .header-left h1 { font-size: 26px; font-weight: 700; margin: 0; line-height: 1.2; color: #fff; }
    .header-left p { margin: 2px 0; font-size: 14px; color: #e8ecf2; }
    .header-right { text-align: right; }
    .header-right h2 { margin: 0; font-size: 20px; font-weight: 700; text-transform: uppercase; letter-spacing: 1px; }
    .header-right p { font-size: 14px; color: #e8ecf2; margin: 4px 0 0; }
    .sheet-inner { padding: 24px 28px 32px; }
    .info-line { display: flex; justify-content: space-between; font-size: 14px; margin-bottom: 8px; }
    ...
    th { background: #f1f4f9; text-align: center; font-weight: 700; }
    ...
    .sign { display: grid; grid-template-columns: repeat(3, 1fr); text-align: center; gap: 24px; margin-top: 50px; }
...
<body>
  <div class="sheet">
      ...
      <div class="header-inner">
        <div class="header-left">
          ...
          <div>
            ...
            <p>Sup Name :
              <strong>{{ $preorder->supplier->supplier ?? 'N?A' }}</strong>
            </p>
            <p>Sup Phone :
              <strong>{{ $preorder->supplier->phone ?? 'N?A' }}</strong>
            </p>
          </div>
        </div>
        <div class="header-right">
          <h2>Challan</h2>
          <p>Fund :
            <strong>{{ $preorder->fund->fund ?? 'N?A' }}</strong>
          </p>
          <p>Sup Address :
            <strong>{{ $preorder->supplier->address ?? 'N?A' }}</strong>
          </p>
        </div>
      </div>
    </header>
    ...
    <div class="sheet-inner">
      <div class="info-line">
        <span><strong>Memo No:</strong> {{ $preorder->memo_no ?? 'N?A' }}</span>
        <span><strong>Date:</strong>
          {{ \Carbon\Carbon::parse($preorder->date ?? now())->format('d-m-Y') }}
        </span>
      </div>
          ...
          @forelse($items as $idx => $item)
            <tr>
              ...
              <td class="col-desc">
                ...
                @php
                  $catCode = $item->product->category->code ?? null;
                  $subCode = $item->product->subcategory->sub_category_code ?? null;
                @endphp
                @if($catCode || $subCode)
                  <div style="font-size:12px;color:#666; margin-top:4px;">
                    @if($catCode) Cat: {{ $catCode }} @endif
                    @if($catCode && $subCode) | @endif
                    @if($subCode) Sub: {{ $subCode }} @endif
                  </div>
                @endif
              </td>

              <td class="col-country">
                {{ $item->product->country->name ?? 'N/A' }}
              </td>
          ...
          <tr>
            ...
            <td class="col-qty">
              ...
              {{ isset($totalQty)
                  ...
                  : rtrim(rtrim(number_format((float)$items->sum('qty'), 2, '.', ''), '0'), '.') }}
            </td>
          </tr>
        </tfoot>
      </table>
