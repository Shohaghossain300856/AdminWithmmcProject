                    ...
                    <form action="{{route('setting.store')}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="card-body">
                            ...
                            @foreach($fields as $key => $filed)
                                <div class="mb-3">
                                    <label for="permission" class="form-label text-capitalize">{{ \Illuminate\Support\Str::of($key)->replace('_', ' ') }} <code>{{$filed['required'] ? "*" : ''}}</></label>
                                    <input type="{{$filed['type']}}" name="{{$key}}" value="{{ old($key, $editData->$key ?? '') }}"  class="form-control @error("$key") is-invalid @enderror"  id="{{$key}}" placeholder="{{$filed['placeholder']}}" {{$filed['required']}}>
                                    @error("$key")
